 ...
 *  Формирует и разбирает 128-байтные пакеты, структура которых
 *  полностью совпадает с описанным протоколом (см. таблицу в ТЗ).
 *
 *  Основные возможности:
 *   • отправка команды скорости/геометрии (EthTrackedSocket::sendCommand)
 *   • приём обратного пакета состояния (EthTrackedSocket::receiveState)
 *
...
struct Packet128
{
  ...
  float    linVel  = 0.0f;   // [10-13] линейная скорость  (м/с)
  float    angVel  = 0.0f;   // [14-17] угловая скорость   (рад/с)
  uint8_t  rsv2[3]{};        // [18-20] резерв

  float    geomPos = 0.0f;   // [21-24] положение геометрии (рад)
  uint8_t  rsv3[4]{};        // [25-28] резерв
  uint8_t  rsv4[4]{};        // [29-32] резерв
  uint8_t  rsv5[8]{};        // [33-40] резерв

  uint8_t  ctrlMode = 0x00;  // [41]  ручной/авто
  uint8_t  autoPos  = 0x00;  // [42]  автомат. контур позиции
